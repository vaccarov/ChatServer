...
class ImageGenerationProcess:
    ...
    def _run_generation_thread(self):
        try:
            ...
            self.loop.call_soon_threadsafe(self.progress_queue.put_nowait, {"status": STATUS_LOADING_MODEL, "model": self.req.model_name})
            # SETUP BASE PIPELINE
            ...
            if self.req.use_refiner:
                ...
                self.loop.call_soon_threadsafe(self.progress_queue.put_nowait, {"status": STATUS_LOADING_MODEL, "model": STATUS_REFINING})
            ...
            for i, image in enumerate(images_or_latents):
                self.loop.call_soon_threadsafe(self.progress_queue.put_nowait, {"status": STATUS_STARTING_IMAGE, "image_number": i + 1, "total_images": self.req.num_images_per_prompt})
                base64_image = _image_to_base64(image)
                self.loop.call_soon_threadsafe(self.progress_queue.put_nowait, {"status": STATUS_SUCCESS, "image_data": base64_image})
