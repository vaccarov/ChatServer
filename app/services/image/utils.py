...
from app.core.constants import DEVICE, VARIANT, LCM_SDXL_MODEL, MODEL_LCM, SDXL_BASE_MODEL, SDXL_REFINER_MODEL
...
class PipelineArguments:
	...
	def get_refiner_args(
		self, images, callback: Callable[[Any, int, Any, Dict[str, Any]], Dict[str, Any]]
	) -> Dict[str, Any]:
...
def _get_common_pipeline_args() -> Dict[str, Any]:
	...
	return {'torch_dtype': torch.float16, 'use_safetensors': True, 'variant': VARIANT}


def _load_sdxl_pipeline(pipeline_class):
	return pipeline_class.from_pretrained(SDXL_BASE_MODEL, **_get_common_pipeline_args()).to(DEVICE)


def _load_lcm_pipeline(pipeline_class):
	unet = UNet2DConditionModel.from_pretrained(LCM_SDXL_MODEL, **_get_common_pipeline_args())
	pipe = pipeline_class.from_pretrained(SDXL_BASE_MODEL, unet=unet, **_get_common_pipeline_args()).to(DEVICE)
...
def _load_refiner(pipe: DiffusionPipeline) -> Optional[DiffusionPipeline]:
	...
	refiner = AutoPipelineForImage2Image.from_pretrained(
		SDXL_REFINER_MODEL, text_encoder_2=pipe.text_encoder_2, vae=pipe.vae, **_get_common_pipeline_args()
	).to(DEVICE)
